...
class AnalisController extends BaseController
{
    ...
    public function store()
    {
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            foreach ($disposisi as $eachDisposisi) {
                foreach ($eachDisposisi['sampel'] as $sampel) {
                    foreach ($sampel['analis'] as $analis) {
                        $formatted[] = [
                            'id_fppc' => $eachDisposisi['fppc_id'],
                            'id_dtl_fppc' => $sampel['dtl_fppc_id'],
                            'id_permohonan_uji' => $sampel['permohonan_uji_id'],
                            'analis_id' => $analis,
                            'penyelia_id' => $adminId,
                        ];
                    }
            ...
            $DisposisiAnalis->insertBatch($formatted);

            $fppc_id = $formatted[0]['id_fppc'];

            $FppcModel->update($fppc_id, ['status' => 'proses-pengujian']);

            $activityDescription = 'Pengujian sedang dalam proses';
            ...
            session()->setFlashdata('success', 'Berhasil membuat disposisi analis');
    ...
    public function createDisposisiViews($id)
    {
        ...
        return view('pages/disposisi-analis-create', [
            'fppc' => $fppcData,
            'title' => 'Disposisi Analis',
            'admin' => $adminData,
